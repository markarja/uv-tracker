...
{ 
	...
	var precalculatedMax = (params.max) ? params.max : 11.0;
	...
	this.config = {
		...
		originalValue: 0.0, //(params.value) ? params.value : (precalculatedMax - precalculatedMin) / 2.0,
		
		// The current gauge value. Might be adopted to prevent underflow/overflow the min/max.
		value: 0.0, //(params.value) ? params.value : (precalculatedMax - precalculatedMin) / 2.0,
		...
		gaugeWidthScale: (params.gaugeWidthScale != undefined) ? params.gaugeWidthScale : 1.2,
		...
		gaugeBackColor: (params.gaugeBackColor != undefined) ? params.gaugeBackColor : "#cfcfcf",
		
		// Hide or display a gauge arc shadow. Dafault is true.
		showGaugeShadow: (params.showGaugeShadow != undefined) ? params.showGaugeShadow : false,
		...
		gaugeShadowScale: (params.gaugeShadowScale != undefined) ? params.gaugeShadowScale : 0,
		...
		actionRangeColor: (params.actionRangeColor != undefined) ? params.actionRangeColor : "#46a700"
	};
...
if (this.config.showGaugeShadow == true)
{
	...
	if (this.config.lowerWarningLimit >= 0.0 && this.config.lowerWarningLimit != this.config.min)
	{
		...
		this.gaugeLWR.setAttributeNS(null, 'fill',   '#fff900');
	...
	if (this.config.lowerWarningLimit >= 0.0 && this.config.upperWarningLimit >= 0.0)
	{
		...
		this.gaugeOPT.setAttributeNS(null, 'fill',   '#e97b00');
	...
	if (this.config.upperWarningLimit >= 0.0 && this.config.upperWarningLimit != this.config.max)
	{
		...
		this.gaugeUWR.setAttributeNS(null, 'fill',   '#d81300');
	...
	if (this.config.upperActionLimit > 0.0)
	{
		...
		this.gaugeUAR.setAttributeNS(null, 'fill',   '#6b49c8');
	...
	this.gaugeVAL.setAttributeNS(null, 'style',  "font-family:Helvetica; font-weight: lighter; font-size:" + Math.floor(this.config.canvasW / 8) + "px; fill-opacity:1.0; fill:" + this.config.valueColor + "; text-anchor:middle;");
	...
	this.refresh = function(valueNew, animated)
	{
		...
		this.gaugeVAL.childNodes[0].textContent = (this.config.value == 11) ? this.config.value + "+" : this.config.value;
		
		var valueColor = "#46a700";
		
		if(this.config.value < 3) {
			valueColor = "#46a700";
		...
		this.gaugeVAL.setAttributeNS(null, 'style',  "font-family:Helvetica; font-weight: lighter; font-size:" + Math.floor(this.config.canvasW / 8) + "px; fill-opacity:1.0; fill: " + valueColor + "; text-anchor:middle;");	
